    ...
    extension ParsedPeers {
        fileprivate init(accountId: Int64, peers: WidgetDataPeers) {
            self.init(
                peers: peers.peers.map { peer -> ParsedPeer in
                    return ParsedPeer(
                        accountId: accountId,
    ...
    private let accountAuxiliaryMethods = AccountAuxiliaryMethods(
        updatePeerChatInputState: { interfaceState, inputState -> PeerChatInterfaceState? in
    ...
    private func getCommonTimeline(
        friends: [Friend]?, in context: TimelineProviderContext,
        completion: @escaping (Timeline<SimpleEntry>) -> Void
    ) {
        if context.isPreview {
            completion(
                Timeline(entries: [SimpleEntry(date: Date(), contents: .preview)], policy: .atEnd))
            return
        }
        ...
        guard let appBundleIdentifier = Bundle.main.bundleIdentifier,
            ...
            completion(
                Timeline(entries: [SimpleEntry(date: entryDate, contents: .recent)], policy: .atEnd)
            )
            return
        }
        ...
        TempBox.initializeShared(
            basePath: rootPath, processType: "widget", launchSpecificId: arc4random64())
        ...
        for (accountId, items) in itemsByAccount {
            friendsByAccount.append(
                accountTransaction(
                    rootPath: rootPath, id: AccountRecordId(rawValue: accountId),
                    encryptionParameters: encryptionParameters, isReadOnly: true, useCopy: true,
                    transaction: { postbox, transaction -> [ParsedPeer] in
                        ...
                        for (peerId, _) in items {
                            ...
                            if let readState = transaction.getCombinedPeerReadState(peer.id),
                                readState.count > 0
                            {
                                var isMuted = false
                                if let notificationSettings =
                                    transaction.getPeerNotificationSettings(peer.id)
                                    as? TelegramPeerNotificationSettings
                                {
                            ...
                            if let index = transaction.getTopPeerMessageIndex(peerId: peer.id) {
                                if let message = transaction.getMessage(index.id) {
                                    mappedMessage = WidgetDataPeer.Message(
                                        accountPeerId: state.peerId, message: message)
                                }
                            }

                            let widgetPeer = WidgetDataPeer(
                                id: peer.id.toInt64(), name: name, lastName: lastName,
                                letters: peer.displayLetters,
                                avatarPath: smallestImageRepresentation(
                                    peer.profileImageRepresentations
                                ).flatMap { representation in
                                    return postbox.mediaBox.resourcePath(representation.resource)
                                }, badge: badge, message: mappedMessage)

                            result.append(
                                ParsedPeer(
                                    accountId: accountId, accountPeerId: state.peerId.toInt64(),
                                    peer: widgetPeer))
                        }
        ...
        let _ = combineLatest(friendsByAccount).start(next: { allPeers in
            ...
            let result = ParsedPeers(
                peers: orderedPeers, updateTimestamp: Int32(Date().timeIntervalSince1970))
            completion(
                Timeline(
                    entries: [SimpleEntry(date: entryDate, contents: .peers(result))],
                    policy: .atEnd))
        })
    }
    ...
    struct Provider: IntentTimelineProvider {
        ...
        func getSnapshot(
            for configuration: SelectFriendsIntent, in context: Context,
            completion: @escaping (SimpleEntry) -> Void
        ) {
            let entry = SimpleEntry(
                date: Date(),
                contents: context.isPreview
                    ? .preview
                    : .peers(
                        ParsedPeers(
                            accountId: 0,
                            peers: WidgetDataPeers(accountPeerId: 0, peers: [], updateTimestamp: 0))
                    ))
            completion(entry)
        }
    ...
    struct AvatarsProvider: IntentTimelineProvider {
        ...
        func getSnapshot(
            for configuration: SelectAvatarFriendsIntent, in context: Context,
            completion: @escaping (SimpleEntry) -> Void
        ) {
            let entry = SimpleEntry(
                date: Date(),
        ...
        var body: some View {
            return ZStack {
                if let peer = peer {
                    Image(
                        uiImage: avatarImage(
                            accountPeerId: peer.accountPeerId, peer: peer.peer,
    ...
    struct WidgetView: View {
        ...
        private func linkForPeer(accountId: Int64, id: Int64) -> String {
            ...
            default:
                return
                    "\(buildConfig.appSpecificUrlScheme)://localpeer?id=\(id)&accountId=\(accountId)"
            }
        }
        ...
        func chatTopLine(_ content: ChatContent) -> some View {
            ...
            case let .peer(peer):
                if let message = peer.peer.message {
                    dateText = DateFormatter.localizedString(
                        from: Date(timeIntervalSince1970: Double(message.timestamp)),
            ...
            case .placeholder:
                ...
                dateText = "         "
                chatTitle = AnyView(
                    Text(" ").font(Font.system(size: 16.0, weight: .medium, design: .default))
                        .foregroundColor(.primary))
                date = Text(dateText)
                            ...
                            .background(
                                GeometryReader { geometry in
                                    RoundedRectangle(cornerRadius: 4.0)
                                        ...
                                        .frame(
                                            width: geometry.size.width, height: 8.0,
                                            alignment: .center
                                        )
                                ...
                                .background(
                                    GeometryReader { geometry in
                                        RoundedRectangle(cornerRadius: 4.0)
                                            ...
                                            .offset(
                                                x: 0.0, y: (geometry.size.height - 8.0) / 2.0 - 1.0)
                                    }
                                )
                            Text(" ")
                                .lineLimit(1)
        ...
        func chatContentView(_ index: Int, size: CGSize) -> AnyView {
                    ...
                    HStack(
                        ...
                        content: {
                            Image("Widget/Avatar\(index == 0 ? "Channel" : "1")")
                                ...
                                .padding(
                                    EdgeInsets(top: 0.0, leading: 10.0, bottom: 0.0, trailing: 10.0)
                                )
                            chatContent(.preview(index)).frame(maxWidth: .infinity).padding(
                                EdgeInsets(top: 0.0, leading: 0.0, bottom: 0.0, trailing: 10.0))
                        }
                    )
                    .frame(width: size.width, height: itemHeight, alignment: .leading)
                )
            ...
            if let peers = peers {
                url = URL(
                    string: linkForPeer(
                        accountId: peers.peers[index].accountId, id: peers.peers[index].peer.id))!
            } else {
            ...
            avatarView = AvatarItemView(
                peer: peers?.peers[index], itemSize: 54.0, placeholderColor: getPlaceholderColor())
                        ...
                        HStack(
                            ...
                            content: {
                                avatarView
                                    ...
                                    .padding(
                                        EdgeInsets(
                                            top: 0.0, leading: 10.0, bottom: 0.0, trailing: 10.0))
                                chatContent(content).frame(maxWidth: .infinity).padding(
                                    EdgeInsets(top: 0.0, leading: 0.0, bottom: 0.0, trailing: 10.0))
                            })
                    }
                )
            ...
            case let .peers(peersValue):
                if peersValue.peers.isEmpty {
                    ...
                    if !calendar.isDate(Date(), inSameDayAs: date) {
                        ...
                        text = self.presentationData.widgetUpdatedAt.replacingOccurrences(
                            of: "{}", with: formatter.string(from: date))
                    } else {
                        let formatter = DateFormatter()
                        formatter.dateStyle = .none
                        formatter.timeStyle = .short
                        text = self.presentationData.widgetUpdatedTodayAt.replacingOccurrences(
                            of: "{}", with: formatter.string(from: date))
                    }
                }
            case .preview:
        ...
        func getBackgroundColor() -> Color {
            ...
            case .dark:
                return Color(
                    .sRGB, red: 28.0 / 255.0, green: 28.0 / 255.0, blue: 30.0 / 255.0, opacity: 1.0)
            @unknown default:
                return .secondary
            }
        }
        ...
        func getSeparatorColor() -> Color {
            ...
            case .light:
                return Color(
                    .sRGB, red: 216.0 / 255.0, green: 216.0 / 255.0, blue: 216.0 / 255.0,
                    opacity: 1.0)
            case .dark:
                return Color(
                    .sRGB, red: 0.0 / 255.0, green: 0.0 / 255.0, blue: 0.0 / 255.0, opacity: 1.0)
            @unknown default:
                return .secondary
            }
        }
        ...
        func getUpdatedBackgroundColor() -> Color {
            ...
            case .light:
                return Color(
                    .sRGB, red: 242.0 / 255.0, green: 242.0 / 255.0, blue: 247.0 / 255.0,
                    opacity: 1.0)
            case .dark:
                return Color(
                    .sRGB, red: 21.0 / 255.0, green: 21.0 / 255.0, blue: 21.0 / 255.0, opacity: 1.0)
            @unknown default:
                return .secondary
            }
        }
        ...
        func getUpdatedTextColor() -> Color {
            ...
            case .light:
                return Color(
                    .sRGB, red: 142.0 / 255.0, green: 142.0 / 255.0, blue: 146.0 / 255.0,
                    opacity: 1.0)
            case .dark:
                return Color(
                    .sRGB, red: 142.0 / 255.0, green: 142.0 / 255.0, blue: 146.0 / 255.0,
        ...
        func getPlaceholderColor() -> Color {
            ...
            case .light:
                return Color(
                    .sRGB, red: 235.0 / 255.0, green: 235.0 / 255.0, blue: 241.0 / 255.0,
                    opacity: 1.0)
            case .dark:
                return Color(
                    .sRGB, red: 38.0 / 255.0, green: 38.0 / 255.0, blue: 41.0 / 255.0, opacity: 1.0)
            @unknown default:
                return .secondary
            }
        }
            ...
            if let peers = peers {
                return AnyView(
                    Link(
                        destination: URL(
                            string: linkForPeer(
                                accountId: peers.peers[index].accountId,
                                id: peers.peers[index].peer.id))!,
                        label: {
                            GeometryReader(content: { geometry in
                                AvatarItemView(
                                    peer: peers.peers[index], itemSize: geometry.size.height,
                ...
                return AnyView(
                    Circle().aspectRatio(1.0, contentMode: .fit).foregroundColor(
                        getPlaceholderColor()))
            } else if isPreview {
                return AnyView(
                    Image("Widget/Avatar\(index + 1)").aspectRatio(1.0, contentMode: .fit)
                        .clipShape(Circle()))
            } else {
                return AnyView(
                    Circle().aspectRatio(1.0, contentMode: .fit).foregroundColor(
    ...
    struct Static_Widget: Widget {
        ...
        public var body: some WidgetConfiguration {
            ...
            return IntentConfiguration(
                kind: kind, intent: SelectFriendsIntent.self, provider: Provider(),
                content: { entry in
                    WidgetView(
                        data: getWidgetData(contents: entry.contents),
    ...
    struct Static_AvatarsWidget: Widget {
        ...
        public var body: some WidgetConfiguration {
            ...
            return IntentConfiguration(
                kind: kind, intent: SelectAvatarFriendsIntent.self, provider: AvatarsProvider(),
                content: { entry in
                    AvatarsWidgetView(
                        data: getWidgetData(contents: entry.contents),
                        presentationData: presentationData)
